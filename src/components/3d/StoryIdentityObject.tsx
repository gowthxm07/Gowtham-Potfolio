...
export function StoryIdentityObject({ progress }: StoryIdentityObjectProps) {
  ...
  useFrame(({ pointer }, delta) => {
    ...
    // Section range for identity visibility: [0.15, 0.50]
    // Peak focus: [0.26, 0.38]
    let targetZ = -8.0;
    let targetX = 0.85;
    let targetY = 0.8;
    let targetScale = 0.5;
    ...
    if (progress >= 0.12 && progress < 0.28) {
      ...
      const t = smoothStep((progress - 0.12) / (0.28 - 0.12));
      ...
      const t = smoothStep((progress - 0.40) / (0.52 - 0.40));
