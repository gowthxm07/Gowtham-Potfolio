...
export function StoryOverlay({ progress }: StoryOverlayProps) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-20 overflow-hidden select-none">
      ...
      <section
        style={introStyle}
        className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 sm:p-6 pt-16 sm:pt-6"
      >
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-mono tracking-widest bg-emerald-950/60 text-emerald-400 border border-emerald-800/60 mb-4 sm:mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            ENGINEERING PORTFOLIO // 2026
          </div>

          <h1 className="text-3xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-white uppercase font-mono mb-3 sm:mb-4">
            {profileData.name}
          </h1>

          <p className="text-sm sm:text-xl text-slate-300 font-sans font-light max-w-2xl mx-auto mb-4 sm:mb-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 text-[11px] sm:text-xs font-mono text-slate-400 mb-6 sm:mb-10">
          ...
          {/* Scroll Cue */}
          <div className="flex flex-col items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-mono text-emerald-400/80 animate-bounce">
      ...
      <section
        style={identityStyle}
        className="absolute inset-x-0 inset-y-0 md:inset-y-0 md:left-0 md:right-auto flex items-center justify-center md:justify-start w-full md:w-1/2 lg:w-5/12 p-3.5 sm:p-6 md:pl-16 pt-16 md:pt-6 pointer-events-none"
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          <div className="text-[10px] sm:text-[11px] font-mono text-emerald-400 tracking-widest uppercase mb-1.5 sm:mb-2 flex items-center gap-2">
            <Terminal className="w-3.5 h-3.5 shrink-0" />
            02 // IDENTITY & STANDING
          </div>

          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold font-mono text-white mb-1.5 sm:mb-2">
            {profileData.name}
          </h2>

          <p className="text-xs sm:text-sm text-slate-300 mb-3 sm:mb-4 leading-relaxed font-sans">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-2.5 mb-4 sm:mb-6">
            <div className="p-2.5 sm:p-3 rounded-lg bg-surface-card border border-surface-border">
              <div className="text-[10px] font-mono text-slate-400">LeetCode Contest</div>
              <div className="text-sm sm:text-base font-bold text-emerald-400 font-mono">1868 Peak</div>
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono">Knight Level (Top 4.96%)</div>
            </div>
            <div className="p-2.5 sm:p-3 rounded-lg bg-surface-card border border-surface-border">
              <div className="text-[10px] font-mono text-slate-400">Problems Solved</div>
              <div className="text-sm sm:text-base font-bold text-white font-mono">900+ Problems</div>
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono">C++ / Algorithms</div>
            </div>
          </div>

          <div className="space-y-1.5 sm:space-y-2 text-xs font-mono text-slate-300 border-t border-surface-border pt-3 sm:pt-4 mb-4 sm:mb-6">
            <div className="flex items-center gap-2">
              <Award className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>DeltaBuild 2026 Winner • CodeRoyale Runner-Up</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3.5 text-center text-emerald-400 font-bold shrink-0">•</span>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-xs font-mono bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/40 text-emerald-300 transition-colors"
            >
      ...
      <section
        style={projectsStyle}
        className="absolute inset-x-0 inset-y-0 md:inset-y-0 md:right-0 md:left-auto flex items-center justify-center md:justify-end w-full md:w-1/2 lg:w-5/12 p-3.5 sm:p-6 md:pr-16 ml-auto pt-16 md:pt-6 pointer-events-none"
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg shadow-2xl w-full max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          {/* Unified Stable Header with Switcher Tabs */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 sm:gap-2 mb-2 pb-2 border-b border-surface-border/50">
            <div
              className={`text-[10px] sm:text-[11px] font-mono tracking-wider sm:tracking-widest uppercase flex items-center gap-1.5 sm:gap-2 truncate transition-colors duration-200 ${
                activeProjectIdx === 5
                  ? "text-purple-400"
                  : activeProjectIdx === 4
                  ? "text-blue-400"
            ...
            >
              ...
              <span className="truncate">
                {activeProjectIdx === 0
                  ...
                  : activeProjectIdx === 2
                  ? "03 // VIDEO CARTOONIFIER"
                  : activeProjectIdx === 3
                  ? "04 // INDUSTRIAL MARKETPLACE"
                  : activeProjectIdx === 4
                  ? "05 // CORPORATE PRINTING"
                  : "06 // SMART HOME ENGINE"}
              </span>
            </div>
            {/* Project Switcher Tabs */}
            <div className="flex gap-1 sm:gap-1.5 bg-surface-card p-1 rounded-lg border border-surface-border shrink-0 self-start sm:self-auto">
              {projectsData.slice(0, 6).map((p, pIdx) => (
                <button
                  key={p.id}
                  onClick={() => scrollToProject(pIdx)}
                  className={`text-[10px] font-mono px-1.5 sm:px-2 py-0.5 rounded transition-colors ${
          ...
          <div data-project-grid className="grid grid-cols-1 grid-rows-1">
            {projectsData.slice(0, 6).map((project, idx) => {
              ...
              return (
                ...
                >
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold font-mono text-white mb-1.5 md:mb-2 leading-tight">
                    {project.title}
                  </h2>

                  <p className="text-xs md:text-sm text-slate-300 mb-2.5 sm:mb-3.5 leading-relaxed font-sans line-clamp-3 sm:line-clamp-4 md:line-clamp-none">
                  ...
                  {/* Spatial Concept Tag */}
                  <div
                    className={`p-2 sm:p-2.5 rounded-lg bg-surface-card border border-surface-border text-[10px] sm:text-[11px] font-mono mb-2.5 sm:mb-3.5 ${anchorTextColor}`}
                  >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2 mb-2.5 sm:mb-3.5">
                    {project.metrics.slice(0, 4).map((m: string, i: number) => (
                      <div key={i} className="p-1.5 sm:p-2 rounded bg-surface-card border border-surface-border text-[10px] sm:text-[11px] text-slate-300 font-sans">
                        <span
                          className={`font-mono block text-[8px] sm:text-[9px] uppercase ${accentTextColor}`}
                        >
                          Telemetry 0{i + 1}
                        </span>
                        {m}
                  ...
                  {/* Tech Badges */}
                  <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-3.5 sm:mb-5">
                    {project.technologies.map((t) => (
                      <span
                        key={t}
                        className="text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 rounded bg-surface-card text-slate-300 border border-surface-border"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[11px] sm:text-xs font-mono transition-colors ${repoBtnColor}`}
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      Inspect Repository
                    </a>
                    {project.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[11px] sm:text-xs font-mono bg-blue-500/15 hover:bg-blue-500/25 border border-blue-500/40 text-blue-300 transition-colors"
                      >
      ...
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          ...
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold font-mono text-white mb-1">
            {profileData.institution}
          </h2>

          <div className="text-xs font-mono text-emerald-400 mb-2.5 sm:mb-3">
            {profileData.degree} • {profileData.period}
          </div>

          <p className="text-xs md:text-sm text-slate-300 mb-3 sm:mb-4 leading-relaxed font-sans">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-2.5 mb-4 sm:mb-5">
            <div className="p-2.5 sm:p-3 rounded-lg bg-surface-card border border-surface-border">
              <div className="text-[9px] sm:text-[10px] font-mono text-slate-400 uppercase">Cumulative GPA</div>
              <div className="text-base sm:text-lg font-bold text-emerald-400 font-mono">{profileData.gpa}</div>
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono">First Class Standing</div>
            </div>
            <div className="p-2.5 sm:p-3 rounded-lg bg-surface-card border border-surface-border">
              <div className="text-[9px] sm:text-[10px] font-mono text-slate-400 uppercase">Campus Role</div>
              <div className="text-[11px] sm:text-xs font-bold text-white font-mono mt-0.5 sm:mt-1">GFG Placement Lead</div>
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono">DSA Peer Mentorship</div>
            </div>
          </div>

          {/* Coursework Focus */}
          <div className="border-t border-surface-border pt-3 sm:pt-4">
            <div className="text-[9px] sm:text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1.5 sm:mb-2">
      ...
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          ...
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold font-mono text-white mb-1.5 sm:mb-2">
            Technical Capabilities & Stack
          </h2>

          <p className="text-xs md:text-sm text-slate-300 mb-2.5 sm:mb-3.5 leading-relaxed font-sans">
          ...
          {/* Core Strengths Bar */}
          <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-2.5 sm:mb-3.5 pb-2 sm:pb-2.5 border-b border-surface-border">
            {coreStrengths.map((strength) => (
              <span
                key={strength}
                className="text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 rounded bg-emerald-950/40 text-emerald-300 border border-emerald-800/50"
              >
                ★ {strength}
              </span>
            ))}
          ...
          {/* Category Tabs */}
          <div className="flex flex-wrap gap-1 mb-2.5 sm:mb-3.5 bg-surface-card/60 p-1 rounded-lg border border-surface-border">
            {skillsData.map((cat) => (
              <button
                key={cat.category}
                onClick={() => setActiveSkillCategory(cat.category)}
                className={`text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 sm:py-1 rounded transition-colors ${
          ...
          {(() => {
            ...
            return (
              <div className="space-y-2 sm:space-y-2.5 mb-2.5 sm:mb-3.5">
                <div className="text-[11px] sm:text-xs text-slate-300 font-sans italic">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-1.5">
                  {currentCat.skills.map((skill) => (
                    <span
                      key={skill}
                      className="text-[10px] sm:text-[11px] font-mono px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md bg-surface-card text-emerald-300 border border-emerald-800/40 flex items-center gap-1.5"
                    >
          ...
          <div className="border-t border-surface-border pt-2.5 sm:pt-3">
            <div className="text-[9px] sm:text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
      ...
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          ...
          <div className="p-3 sm:p-3.5 rounded-xl bg-surface-card border border-surface-border mb-2.5 sm:mb-3.5">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 text-center my-2 sm:my-2.5">
              <div className="bg-surface/60 p-1.5 rounded border border-surface-border">
                <div className="text-[8px] sm:text-[9px] font-mono text-slate-400">RATING</div>
                <div className="text-xs sm:text-sm font-bold text-emerald-400 font-mono">1868</div>
              </div>
              <div className="bg-surface/60 p-1.5 rounded border border-surface-border">
                <div className="text-[8px] sm:text-[9px] font-mono text-slate-400">SOLVED</div>
                <div className="text-xs sm:text-sm font-bold text-white font-mono">900+</div>
              </div>
              <div className="bg-surface/60 p-1.5 rounded border border-surface-border">
                <div className="text-[8px] sm:text-[9px] font-mono text-slate-400">GLOBAL</div>
                <div className="text-xs sm:text-sm font-bold text-emerald-400 font-mono">Top 4.96%</div>
              </div>
            </div>
          ...
          <div className="space-y-2 sm:space-y-2.5">
            <div className="p-2.5 sm:p-3 rounded-lg bg-surface-card border border-surface-border">
              ...
              <div className="text-[9px] sm:text-[10px] font-mono text-slate-400 mb-1">
      ...
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          ...
          <div className="p-3 sm:p-3.5 rounded-xl bg-surface-card border border-surface-border space-y-1.5 sm:space-y-2 mb-4 sm:mb-5 text-[11px] sm:text-xs font-mono">
            ...
            <div className="flex items-center justify-between text-slate-300">
              ...
              <span className="text-emerald-400 truncate ml-2 text-right">{profileData.degree} • GPA {profileData.gpa}</span>
            </div>
            <div className="flex items-center justify-between text-slate-300">
              <span className="text-slate-400">Competitive:</span>
              <span className="text-emerald-300">LeetCode Knight [1868]</span>
            </div>
            <div className="flex items-center justify-between text-slate-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs font-mono bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/50 text-emerald-300 font-bold transition-colors"
            >
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs font-mono bg-surface hover:bg-slate-800 border border-slate-700 text-slate-300 transition-colors"
            >
      ...
      <section
        style={contactStyle}
        className="absolute inset-x-0 inset-y-0 md:inset-y-0 md:right-0 md:left-auto flex items-center justify-center md:justify-end w-full md:w-1/2 lg:w-6/12 p-3.5 sm:p-6 md:pr-16 ml-auto pt-16 md:pt-6 pointer-events-none"
      >
        <div className="bg-surface/90 border border-surface-border backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-8 max-w-xl w-full shadow-2xl max-h-[calc(100dvh-5.5rem)] overflow-y-auto pointer-events-auto box-border">
          <div className="text-[10px] sm:text-[11px] font-mono text-emerald-400 tracking-widest uppercase mb-1.5 sm:mb-2 flex items-center gap-2">
          ...
          <p className="text-xs text-slate-300 mb-3 sm:mb-4 font-sans leading-relaxed">
            Have a project, opportunity, or idea? Let's build something meaningful. Direct inquiries route to{" "}
            <span className="text-emerald-400 font-mono break-all">{profileData.email}</span>.
          </p>
